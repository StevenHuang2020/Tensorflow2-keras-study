# python3 Steven Autoencoder mnist
...
def getData(batch_size = 32):
    def preprocess_images(images):
        ...
        return np.where(images > .5, 1.0, 0.0).astype('float32') #binary image

    (train_images, _), (test_images, _) = ks.datasets.mnist.load_data()
    ...
    return train_dataset,test_dataset, train_images.shape[0]//batch_size
...
def generate_and_save_images(model, epoch, test_sample):
    ...
    plt.savefig(r'./imagesAuto/image_at_epoch_{:04d}.png'.format(epoch))
    plt.show()

def train():
    batch_size = 32
    train_dataset, test_dataset, batchs = getData(batch_size=batch_size)
    epochs = 10
    ...
    pb = SimpleProgressBar(total=batchs)
